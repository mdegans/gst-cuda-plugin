...
static Suite* dsdistance_suite(void) {
  ...
  // FIXME(mdegans): make these work. right now the plugin fails pretty hard when no metadata
  //  is attached to the buffer. this needs to be fixed since these these tests don't pass.
  //  instead, a GST_WARNING should be printed on no metadata.
