...
/**
 * SECTION:element-payloadbroker
 *
 * PayloadBroker is an element for coloring osd boxes with DeepStream.
 *
 * PayloadBroker operates on metadata only.
 *
 * <refsect2>
 * <title>Example usage</title>
 * |[
 * ... ! nvinfer ! nvtracker ! payloadbroker ! nvosd ...
 * ]|
 * </refsect2>
 */
...
GST_DEBUG_CATEGORY_STATIC(gst_payloadbroker_debug);
#define GST_CAT_DEFAULT gst_payloadbroker_debug
...
#define gst_payloadbroker_parent_class parent_class
G_DEFINE_TYPE(GstPayloadBroker, gst_payloadbroker, GST_TYPE_BASE_TRANSFORM);

static void gst_payloadbroker_set_property(GObject* object,
                                           guint prop_id,
                                           const GValue* value,
                                           GParamSpec* pspec);
static void gst_payloadbroker_get_property(GObject* object,
                                           guint prop_id,
                                           GValue* value,
                                           GParamSpec* pspec);

static GstFlowReturn gst_payloadbroker_transform_ip(GstBaseTransform* base,
                                                    GstBuffer* outbuf);
...
/* initialize the payloadbroker's class */
static void gst_payloadbroker_class_init(GstPayloadBrokerClass* klass) {
  ...
  gobject_class->set_property = gst_payloadbroker_set_property;
  gobject_class->get_property = gst_payloadbroker_get_property;
  ...
  GST_BASE_TRANSFORM_CLASS(klass)->transform_ip =
    GST_DEBUG_FUNCPTR(gst_payloadbroker_transform_ip);
  GST_BASE_TRANSFORM_CLASS(klass)->start =
    GST_DEBUG_FUNCPTR(gst_payloadbroker_start);
  GST_BASE_TRANSFORM_CLASS(klass)->stop =
    GST_DEBUG_FUNCPTR(gst_payloadbroker_stop);

  /* debug category for fltering log messages
   */
  GST_DEBUG_CATEGORY_INIT(gst_payloadbroker_debug, ELEMENT_NAME, 0,
                          ELEMENT_DESCRIPTION);
}
 ...
 */
static void gst_payloadbroker_init(GstPayloadBroker* self) {
  GST_DEBUG("payloadbroker init");
